...
<xsl:stylesheet version="1.0"
    ...
    <!--================================= dialogQuote =================================-->
    <!--意向报价的主数据,没有利率-->
    <xsl:template name="master-marketMakingQuote-withoutRate">
    ...
    <!--意向报价的主数据,具有利率-->
    <!--1.price 需要转化为rate-->
    <xsl:template name="master-marketMakingQuote-withRate">
        <xsl:for-each select="message/body/field[@name]">
            <xsl:variable name="nodeName" select="java:XsltUtil.getDialogQuoteMap(@name)"/>
            <xsl:if test="@tag != 453 and @tag != 711 and @tag != 232 and @tag != 136 and @tag != 10210 and @tag != 555
            and @tag != 215 and @tag != 10204">
    ...
    <xsl:template name="master-marketMakingQuote-NoStipulations">
    ...
    <xsl:template name="master-marketMakingQuote-NoMiscFees">
